	...
	switch (action.type) {
		...
		case 'TOGGLE_BASKETITEM':
			...
			let newList = state.list.map(basketItem =>{
				if (basketItem.id === action.id){
					if (basketItem.count == 1) {
						flag = true;
						// return {...basketItem ,bought:!basketItem.bought}
					}
					return {...basketItem ,count: --basketItem.count}
				} else{
					return basketItem
				}
			});
			if (flag){
				newList = state.list.filter(basketItem => basketItem.id !== action.id)
			}
